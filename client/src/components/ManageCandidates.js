...
const ManageCandidates = () => {
  ...
  const fetchCandidates = async () => {
    ...
    setError("");
    try {
      const response = await axios.get("https://voteguard-backend.onrender.com/api/candidates/candidates");
  ...
  const handleDeleteCandidate = async (candidateId) => {
    if (window.confirm("Are you sure you want to delete this candidate?")) {
      try {
        await axios.delete(`https://voteguard-backend.onrender.com/api/candidates/candidates/${candidateId}`);
        fetchCandidates(); 
      } catch (err) {
