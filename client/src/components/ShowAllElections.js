...
const ShowAllElections = () => {
  ...
  const fetchCandidate = async (userId) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/candidates/candidates/${userId}`);
  ...
  const handleRegisterForElection = async (electionId) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/elections/elections/register", {
        electionId,
