...
const RegisterCandidate = () => {
  ...
  useEffect(() => {
    if (!user) {
      ...
      const fetchUserId = async () => {
        try {
          const response = await axios.get(`https://voteguard-backend.onrender.com/api/users/by-email/${user.email}`);
  ...
  const handlePayment = async (event) => {
    ...
    try {
      const { data } = await axios.post("https://voteguard-backend.onrender.com/api/payment-intent", { amount: 50 });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("https://voteguard-backend.onrender.com/api/candidates/candidates/register", {
        userId,
        firstName,
        lastName,
        party,
